...
const SidebarAdmin = () => {
  ...
  return (
    <div className={`bg-[#4E73DF] px-[25px] h-screen ${isSidebarOpen ? 'w-[200px]' : 'w-[60px]'} transition-all`}>
      <div className='py-[30px] flex items-center justify-between border-b-[1px] border-[#EDEDED]/[0.3]'>
        {/* <img src={logo} alt="Logo" className={`h-20 ${isSidebarOpen ? 'block' : 'hidden'}`} /> */}
        <h1 className={`text-white text-[20px] leading-[24px] font-extrabold text-center ${isSidebarOpen ? 'block' : 'hidden'}`}>
          Skanic Library
        </h1>
        {!isSidebarOpen && <img src={logo} alt="Logo" className="h-8 w-8" />}
      </div>
